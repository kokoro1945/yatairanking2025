const FORM_URL = ''; // TODO: 2026年度用のGoogleフォームURLを設定
const GOOGLE_FORM_ENABLED = Boolean(FORM_URL);
const STORAGE_KEY = 'voted_booths_2026';
...
const SUPABASE_TABLE = 'yatai_votes_2026';
...
function handleFormSubmit(event) {
  ...
  if (GOOGLE_FORM_ENABLED) {
    requests.push(fetch(FORM_URL, {
      method: 'POST',
      mode: 'no-cors',
      body: formData,
    }));
    requestLabels.push('google');
  } else {
    console.info('[submit] Google Form submission is disabled for 2026.');
  }

  if (supabasePayload) {
    // Google フォーム送信と並列で Supabase への登録を実行
    requests.push(sendToSupabase(supabasePayload));
    requestLabels.push('supabase');
  }

  if (!requests.length) {
    showAlert('送信先が未設定です。運営にお問い合わせください。', 'error');
  ...
  Promise.allSettled(requests).then((results) => {
    ...
    const googleResult = results[requestLabels.indexOf('google')];
    const supabaseResult = results[requestLabels.indexOf('supabase')];
