...
## 8. 2025-10 QRコード導線改修（概要）

- トップページ来訪時に QR コード読み取り案内を主導線とし、屋台番号入力フォームはサブ導線として折りたたみ表示に切り替える。
- `?booth=***` 付き URL からのアクセス時は従来どおり屋台番号入力を経由せずフォームに遷移させる。
- 実装の詳細要件は `docs/qr_entry_guidance_update.md` を参照。
